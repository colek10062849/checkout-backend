...
// Firebase Admin for server-side revocation
// Requires GOOGLE_APPLICATION_CREDENTIALS env var pointing to service account JSON
// OR set FIREBASE_SERVICE_ACCOUNT env var as JSON string
let adminDb = null;
try {
...
app.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.type === 'payment_intent.succeeded') {
    ...
    // Mark payment as confirmed in Firestore if we have admin access
    if (adminDb && pi.metadata?.email) {
      try {
        // Clear any pending revocation flag for this payment
        const codesSnap = await adminDb.collection('provisioningCodes')
          .where('paymentId', '==', pi.id)
          .limit(1).get();
        if (!codesSnap.empty) {
          await codesSnap.docs[0].ref.update({ paymentConfirmed: true, paymentStatus: 'paid' });
        }
  ...
  if (event.type === 'payment_intent.payment_failed') {
    ...
    // If a code was already issued for this payment, trigger revocation
    if (adminDb) {
      try {
        const codesSnap = await adminDb.collection('provisioningCodes')
          .where('paymentId', '==', pi.id)
        ...
        if (!codesSnap.empty) {
          ...
          const redeemedBy = codeData.redeemedBy || codeData.soldTo;
          if (redeemedBy) {
            // Set pendingRevokeCode on the user document — the app picks this up
            const usersSnap = await adminDb.collection('users')
              .where('email', '==', codeData.soldTo)
              .limit(1).get();
            if (!usersSnap.empty) {
              await usersSnap.docs[0].ref.update({
                pendingRevokeCode: codeDoc.id
              });
              console.log('Revocation queued for code:', codeDoc.id);
            }
            // Also mark the code itself
      ...
      } catch(e) { console.warn('Revocation webhook failed:', e.message); }
    }
  }
  ...
  if (event.type === 'charge.refunded') {
    ...
    // Same revocation flow as payment failure
      ...
      try {
        ...
        if (!codesSnap.empty) {
          ...
          if (codeData.soldTo) {
            ...
            await codeDoc.ref.update({ refunded: true, paymentStatus: 'refunded' });
          }
        }
      } catch(e) { console.warn('Refund revocation failed:', e.message); }
    }
  }
...
app.post('/contact', async (req, res) => {
  try {
    ...
    // Store in Firestore via Admin SDK would be ideal, but for now return ok
    // The client stores directly to Firestore using their own auth
    console.log('Contact submission from:', email, '| message length:', message.length);
    res.json({ ok: true });
  } catch(err) {
